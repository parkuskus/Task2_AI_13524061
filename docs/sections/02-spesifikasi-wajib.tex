...
Permasalahan ini dibatasi oleh enam \textit{constraint} yang merepresentasikan kondisi neraca keuangan riil Indonesia, mencakup aspek domestik maupun eksternal. \textit{Constraint} C1 dan C2 bersifat struktural terhadap representasi state itu sendiri sehingga dapat diverifikasi langsung dari vektor $s$ tanpa perlu simulasi. \textit{Constraint} C3 sampai C6 melibatkan variabel makroekonomi hasil simulasi sehingga pelanggarannya baru dapat dideteksi setelah fungsi \texttt{compute\_objective(s)} dijalankan.
...
\textbf{C2. Gradualisme Kebijakan (\textit{Interest Rate Smoothing}).}

\[
    \left| r_t - r_{t-1} \right| \leq 50 \text{ bps} \qquad \forall t
\]

Bank sentral secara empiris menghindari perubahan suku bunga yang terlalu agresif karena dapat mengguncang ekspektasi pasar dan merusak kredibilitas \textit{forward guidance} \cite{woodford2003}. \textit{Constraint} ini membatasi perubahan antar kuartal maksimum sebesar 50 basis poin atau 0,50\%.

\textbf{C3. Selisih Minimum terhadap Fed Funds Rate.}

\[
    r_t - r_t^{US} \geq \Delta_{min}, \qquad \Delta_{min} = 1{,}50\%
\]

Selisih suku bunga BI-Rate terhadap Fed Funds Rate tidak boleh turun di bawah $\Delta_{min} = 1{,}50\%$ untuk mencegah \textit{capital outflow} besar-besaran. Jika selisih terlalu kecil, imbal hasil aset rupiah menjadi kurang menarik bagi investor global, yang dapat memicu pelarian modal keluar dari pasar keuangan domestik. Nilai $r_t^{US} = 3{,}625\%$ sesuai midpoint Fed Funds Rate per Juli 2026.

\textbf{C4. Batas Defisit Transaksi Berjalan.}
...
\textbf{C5. Batas \textit{Spread} Imbal Hasil Obligasi Pemerintah.}
...
\textbf{C6. Konvergensi Target Inflasi di Akhir Horizon.}
...
\[
    J_{penalized}(s) = -L(s) - \sum_{i=3}^{6} \mu_i \cdot \max\!\big(0,\; \text{violation}_i(s)\big)^2
\]
...
Suku kedua adalah penalti untuk \textit{constraint} C3 sampai C6 yang dilanggar. Pendekatan \textit{penalty method} dipilih karena \textit{constraint} C3 sampai C6 melibatkan variabel hasil simulasi sehingga tidak dapat diverifikasi sebelum simulasi dijalankan, berbeda dengan C1 dan C2 yang dapat dicek langsung dari vektor $s$. Setiap \textit{constraint} $i \in \{3, 4, 5, 6\}$ memiliki pelanggaran $\text{violation}_i(s)$ yang dihitung sebagai berikut.

\begin{align*}
    \text{violation}_3(s) & = \max_{t}\!\big(0,\ \Delta_{min} - (r_t - r_t^{US})\big)  \\
    \text{violation}_4(s) & = \max_{t}\!\big(0,\ -\theta - CA_t\big)                   \\
    \text{violation}_5(s) & = \max_{t}\!\big(0,\ (y_t^{SBN} - r_t) - \sigma_{max}\big) \\
    \text{violation}_6(s) & = \max\!\big(0,\ |\pi_T - \pi^{*}| - 1\%\big)
\end{align*}

Pelanggaran dikuadratkan agar pelanggaran besar dikenakan penalti yang jauh lebih berat daripada pelanggaran kecil, mendorong algoritma untuk memprioritaskan perbaikan pada \textit{constraint} yang paling parah dilanggar. Bobot penalti $\mu_i$ ditetapkan sebesar 100 untuk seluruh $i$, cukup besar untuk mendominasi perbaikan pada $L(s)$ sehingga algoritma terdorong kuat menuju region \textit{feasible} sebelum melakukan \textit{fine-tuning} terhadap nilai $J(s)$. State yang memenuhi seluruh \textit{constraint} memiliki suku penalti nol sehingga $J_{penalized}(s) = -L(s)$.


\section{Formulasi \textit{Initial State}}

Initial state $s_0$ dibentuk secara acak dengan titik awal $r_0$ mengacu pada kondisi aktual BI-Rate per Juli 2026. Nilai $r_0$ bersifat tetap dan bukan bagian dari state yang dapat dioptimasi. Proses pembangkitan bekerja secara sekuensial. Untuk setiap $t$ dari 1 sampai $T$, dipilih secara acak perubahan $\Delta$ dalam basis poin dari himpunan $\{-50, -25, 0, +25, +50\}$. Nilai $r_t$ diperoleh dari $r_{t-1} + \Delta$ yang kemudian dijepit ke rentang $[r_{min}, r_{max}]$ dan dibulatkan ke kelipatan 0,25\% terdekat. \textit{Constraint} C1 dan C2 secara implisit terpenuhi oleh mekanisme ini. Jika hasil pembangkitan acak tidak memenuhi C3 sampai C6, state tetap diterima sebagai initial state dalam status \textit{infeasible}, dan proses \textit{local search} bertugas mengarahkan pencarian menuju state yang \textit{feasible} sekaligus optimal melalui mekanisme penalti pada fungsi objektif.

\medskip
\noindent\textbf{Kondisi awal sistem (periode $t = 0$).}
\begin{align*}
    r_0      & = 5{,}75\%                &  & \text{BI-Rate Juli 2026 (tetap, bukan bagian state)} \\
    \pi_0    & = 3{,}34\%                &  & \text{Inflasi tahunan Juni 2026}                     \\
    y_0      & = 0{,}00\%                &  & \text{\textit{Output gap} awal (asumsi netral)}      \\
    e_0      & = 17{.}885\ \text{Rp/USD} &  & \text{Nilai tukar 21 Juli 2026}                      \\
    r_0^{US} & = 3{,}625\%               &  & \text{Midpoint Fed Funds Rate (3,50\%--3,75\%)}
\end{align*}
...
\begin{itemize}
    ...
    \item $\text{RMSD}_{\pi} \geq 1{,}00$ dinilai buruk karena rata-rata deviasi inflasi melampaui batas toleransi resmi, mengindikasikan algoritma terjebak pada solusi yang tidak memenuhi \textit{constraint} C6 atau gagal mengeksplorasi ruang state secara memadai.
\end{itemize}
...
Memvalidasi seluruh \textit{constraint} C1 sampai C6 terhadap sebuah state dan \textit{trajectory} simulasinya, serta menghitung besar pelanggaran untuk penalti fungsi objektif.

\begin{itemize}
    \item \texttt{s} (list[float]) state suku bunga.
    \item \texttt{trajectory} (dict) hasil simulasi dari \texttt{simulate\_economy}.
    \item \texttt{params} (dict) parameter \textit{constraint} ($r_{min}, r_{max}, r_0, \Delta_{min}, r_0^{US}, \theta, \sigma_{max}, \pi^{*}$).
    \item \textit{Return} \texttt{dict} berisi \texttt{feasible} (bool) dan \texttt{violations} (dict) dengan kunci \texttt{C3, C4, C5, C6} yang bernilai besar pelanggaran masing-masing. C1 dan C2 dapat diverifikasi langsung dari \texttt{s} tanpa \texttt{trajectory}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textit{Return} \texttt{float} nilai $J_{penalized}(s) = -L(s) - \mu \sum_{i=3}^{6} \text{violation}_i(s)^2$.
\end{itemize}
...
Membentuk initial state $s_0$ secara acak dari $r_0 = 5{,}75\%$ dengan mekanisme \textit{random walk} yang menjaga C1 dan C2.
...
Menghasilkan satu state tetangga dari $s$ menggunakan \textit{Move 1} atau \textit{Move 2} yang dipilih secara acak. Hasil modifikasi diperbaiki terhadap C1 dan C2 sebelum dikembalikan.
...
params = {
    ...
    "rho_y": 0.60, "beta": 0.30, "kappa": 0.11, "phi": 0.15,
    "rho_pi": 0.60, "rho_e": 0.00, "r_star": 2.5,
