...
\textbf{C3. Batas Defisit Transaksi Berjalan.}
...
\textbf{C4. Batas \textit{Spread} Imbal Hasil Obligasi Pemerintah.}
...
\textbf{C5. Konvergensi Target Inflasi di Akhir Horizon.}
...
\[
    J_{penalized}(s) = -L(s) - \sum_{i=3}^{5} \mu_i \cdot \max\!\big(0,\; \text{violation}_i(s)\big)^2
\]
...
Suku kedua adalah penalti untuk \textit{constraint} C3 sampai C5 yang dilanggar. Pendekatan \textit{penalty method} dipilih karena \textit{constraint} C3 sampai C5 melibatkan variabel hasil simulasi sehingga tidak dapat diverifikasi sebelum simulasi dijalankan, berbeda dengan C1 dan C2 yang dapat dicek langsung dari vektor $s$. Setiap \textit{constraint} $i \in \{3, 4, 5\}$ memiliki pelanggaran $\text{violation}_i(s)$ yang dihitung sebagai berikut.

\begin{align*}
    \text{violation}_3(s) & = \max_{t}\!\big(0,\ -\theta - CA_t\big)                   \\
    \text{violation}_4(s) & = \max_{t}\!\big(0,\ (y_t^{SBN} - r_t) - \sigma_{max}\big) \\
    \text{violation}_5(s) & = \max\!\big(0,\ |\pi_T - \pi^{*}| - 1\%\big)
\end{align*}
...
\begin{itemize}
    ...
    \item \textit{Return} \texttt{dict} berisi \texttt{feasible} (bool) dan \texttt{violations} (dict) dengan kunci \texttt{C3, C4, C5} yang bernilai besar pelanggaran masing-masing. C1 dan C2 dapat diverifikasi langsung dari \texttt{s} tanpa \texttt{trajectory}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textit{Return} \texttt{float} nilai $J_{penalized}(s) = -L(s) - \mu \sum_{i=3}^{5} \text{violation}_i(s)^2$.
\end{itemize}
